...
export default function StudentProfilePage() {
  ...
  return (
    <div className="max-w-4xl mx-auto pb-40">
      ...
      {/* ── Profile Header Card ── */}
      <div className="bg-[#D8D2BC] px-5 md:px-10 pt-0 pb-6">
        {/* Avatar row — sits half over the banner */}
        <div className="-mt-16 mb-6 flex items-end justify-between">
          <div className="h-20 w-20 md:h-24 md:w-24 rounded-2xl bg-[#FD5C05] flex items-center justify-center shadow-lg border-4 border-[#D8D2BC] shrink-0 overflow-hidden">
            {currentUser.avatar && (currentUser.avatar.startsWith('data:') || currentUser.avatar.startsWith('http') || currentUser.avatar.startsWith('/')) ? (
              ...
              <span className="text-2xl md:text-3xl font-extrabold text-[#2A2621]">{currentUser.avatar || '🎓'}</span>
            )}
          </div>
          {savedFeedback && (
            <span className="inline-flex items-center gap-1 px-3 py-1.5 rounded-full bg-[#FD5C05]/20 border border-[#FD5C05]/40 text-[10px] font-bold text-[#3a5200] uppercase tracking-wider">
        ...
        <div className="space-y-1 mb-4">
          <h1 className="text-2xl font-extrabold text-[#2A2621] uppercase tracking-tight leading-tight" style={{ fontFamily: 'var(--font-display)' }}>
            {currentUser.name}
          </h1>
          <p className="text-sm text-[#5A554E]">@{currentUser.username}</p>
        </div>

        {/* Badges */}
        <div className="flex flex-wrap gap-3.5 mt-3">
          <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-white border border-black/[0.07] text-[10px] font-semibold text-[#5A554E]">
            <Award className="h-3 w-3 text-[#FD5C05]" />
            {currentUser.major || 'Undeclared'}
          </span>
          <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-white border border-black/[0.07] text-[10px] font-semibold text-[#5A554E]">
            <Calendar className="h-3 w-3 text-[#FD5C05]" />
            Class of {currentUser.graduationYear}
          </span>
          <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-white border border-black/[0.07] text-[10px] font-semibold text-[#5A554E]">
            <Shield className="h-3 w-3 text-[#FD5C05]" />
            {currentUser.school}
          </span>
        </div>
        ...
        <div className="mt-6 pt-6 border-t border-black/[0.05] text-left space-y-2">
          <div className="text-[10px] font-black text-[#2A2621] uppercase tracking-widest">Biography</div>
          <p className="text-xs text-[#5A554E] leading-relaxed max-w-2xl">{bio}</p>
          
          {/* Social links */}
          <div className="flex gap-4 pt-2">
            {socials.linkedin && (
              <a href={socials.linkedin} target="_blank" rel="noreferrer" className="text-xs font-bold text-[#5A554E] hover:text-[#3B5C00] transition-colors flex items-center gap-1">
                LinkedIn ↗
              </a>
            )}
            {socials.github && (
              <a href={socials.github} target="_blank" rel="noreferrer" className="text-xs font-bold text-[#5A554E] hover:text-[#3B5C00] transition-colors flex items-center gap-1">
                GitHub ↗
              </a>
            )}
            {socials.instagram && (
              <a href={socials.instagram} target="_blank" rel="noreferrer" className="text-xs font-bold text-[#5A554E] hover:text-[#3B5C00] transition-colors flex items-center gap-1">
                Instagram ↗
              </a>
            )}
          </div>
        ...
        <div className="grid sm:grid-cols-2 gap-6 mt-6 pt-6 border-t border-black/[0.05] text-left">
          ...
          <div className="space-y-2">
            <div className="text-[10px] font-black text-[#2A2621] uppercase tracking-widest">Areas of Interest</div>
            <div className="flex flex-wrap gap-2">
              {interests.map(item => (
                <span key={item} className="px-2.5 py-1 rounded-lg bg-black/[0.03] border border-[#D8D2BC]/30 text-[10px] text-[#5A554E] font-bold uppercase">
                  {item}
                </span>
              ))}
            </div>
          ...
          {/* Achievements */}
          <div className="space-y-2">
            <div className="text-[10px] font-black text-[#2A2621] uppercase tracking-widest">Achievements & Badges</div>
            <div className="flex flex-wrap gap-2">
              {achievements.map(badge => (
                <span key={badge} className="px-2.5 py-1 rounded-lg bg-[#FD5C05]/10 border border-[#FD5C05]/30 text-[10px] text-[#3B5C00] font-black uppercase tracking-wider flex items-center gap-1">
                  ★ {badge}
                </span>
              ))}
            </div>
      ...
      <div className="px-5 md:px-10 py-4">
        <div className="grid grid-cols-3 gap-3">
          {[
            { label: 'Attended', value: totalRsvps, Icon: CalendarCheck, color: '#FD5C05', bg: '#FD5C0518', route: '/student/my-events' },
            { label: 'Hosted', value: totalCreated, Icon: Star, color: '#2A2621', bg: '#2A262110', route: '/student/my-events' },
            { label: 'Saved', value: totalSaved, Icon: BookOpen, color: '#5A554E', bg: '#5A554E12', route: '/student/saved' },
          ].map(stat => (
            <motion.div
              key={stat.label}
              whileTap={{ scale: 0.95 }}
              ...
              className="bg-white rounded-2xl p-4 flex flex-col items-center gap-2 shadow-sm border border-black/[0.04] transition-all hover:border-[#FD5C05]/30 cursor-pointer select-none"
            >
              <div className="h-8 w-8 rounded-xl flex items-center justify-center" style={{ background: stat.bg }}>
                <stat.Icon className="h-4 w-4" style={{ color: stat.color }} />
              </div>
              <div className="text-2xl font-extrabold text-[#2A2621]">{stat.value}</div>
              <div className="text-[9px] font-bold text-[#5A554E] uppercase tracking-widest">{stat.label}</div>
            </motion.div>
          ))}
        </div>
      </div>
      ...
      <div className="px-5 md:px-10 grid md:grid-cols-2 gap-5">
        ...
        <div className="space-y-3">
          <div className="text-[11px] font-black text-[#5A554E] flex items-center gap-2 uppercase tracking-widest">
            <Users className="h-3.5 w-3.5 text-[#FD5C05]" /> My Organizations
          </div>
          {myOrgs.length > 0 ? (
            <div className="space-y-2">
              {myOrgs.map((org) => {
                  ...
                  >
                    <div className="flex items-center gap-3.5 min-w-0">
                      ...
                      <div className="min-w-0">
                        <p className="font-bold text-[#2A2621] text-sm uppercase tracking-tight group-hover:text-[#3B5C00] transition-colors truncate">
                          {org.name}
                        </p>
                        <div className="flex items-center gap-2 mt-0.5">
                          <span className="px-1.5 py-0.5 rounded-md bg-black/[0.04] text-[#5A554E] text-[8px] font-bold uppercase tracking-wider">
                            {userRole}
                          </span>
                    ...
                    <span className="text-[10px] font-bold text-[#5A554E] group-hover:text-[#2A2621] pl-2 transition-colors">Manage →</span>
                  </div>
                );
              })}
            </div>
            ...
            <div className="bg-white rounded-2xl p-6 text-center border border-black/[0.04]">
              <Users className="h-8 w-8 text-[#5A554E]/30 mx-auto mb-2" />
              <p className="text-xs text-[#5A554E]">Not a member of any organizations yet.</p>
            </div>
          )}
          ...
          {(() => {
            ...
            return (
              <div className="bg-white rounded-2xl p-5 border border-black/[0.04] shadow-sm space-y-4 mt-3 text-left">
                <div className="text-[10px] font-black text-[#2A2621] uppercase tracking-wider">Apply for Membership</div>
                ...
                <div className="flex flex-col sm:flex-row gap-2">
                  <input
                    ...
                    className="flex-1 bg-black/[0.03] border border-black/[0.08] rounded-xl px-3 py-2 text-xs text-[#2A2621] focus:outline-none focus:border-[#FD5C05] transition-colors"
                  />
                    ...
                    <button
                      ...
                      className={`px-3 py-2 rounded-xl text-[9px] font-bold uppercase tracking-wider border transition-all ${
                        orgFilterVerified === null 
                          ...
                          : 'bg-white border-black/10 text-[#5A554E] hover:bg-slate-50'
                      }`}
                    >
                      All
                    </button>
                ...
                {filteredJoinable.length > 0 ? (
                    ...
                    {filteredJoinable.map(org => (
                      <div key={org.id} className="bg-slate-50 border border-black/[0.04] rounded-xl p-3.5 flex items-center justify-between shadow-sm">
                        <div className="flex items-center gap-3 min-w-0">
                          ...
                          <div className="min-w-0">
                            <h4 className="text-xs font-bold text-[#2A2621] uppercase tracking-tight truncate flex items-center">
                              {org.name}
                        ...
                        <button
                          ...
                          className="bg-[#FD5C05] text-[#2A2621] hover:bg-[#c5ff0a] px-3.5 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-wider cursor-pointer transition-all shadow-sm shrink-0"
                        >
                          Apply
                        </button>
                      </div>
                  ...
                  <p className="text-[10px] text-[#5A554E] italic text-center py-2">No joinable campus groups match your query.</p>
                )}
              </div>
            );
          })()}
          ...
          {(() => {
            ...
            return (
              <div className="space-y-2 mt-4">
                <h3 className="text-[9px] font-bold text-[#2A2621] uppercase tracking-wider block pl-1">Pending Applications</h3>
                {pendingUserRequests.map(req => (
                  <div key={req.id} className="bg-white rounded-2xl p-4 flex items-center justify-between border border-amber-500/20 bg-amber-500/[0.02] shadow-sm">
                    <div className="min-w-0 flex-1 pr-3">
                      <p className="font-bold text-[#2A2621] text-xs uppercase tracking-tight truncate">{req.orgName}</p>
        ...
        <div className="space-y-3">
          <div className="text-[11px] font-black text-[#5A554E] flex items-center gap-2 uppercase tracking-widest">
            <Settings className="h-3.5 w-3.5 text-[#5A554E]" /> Account Actions
          </div>
          <div className="bg-white rounded-2xl overflow-hidden divide-y divide-black/[0.05] border border-black/[0.04] shadow-sm">
            ...
            >
              <div className="h-9 w-9 rounded-xl bg-[#FD5C05]/15 flex items-center justify-center shrink-0">
                <Edit3 className="h-4 w-4 text-[#3a5200]" />
              </div>
              <div className="flex-1 text-left">
                <p className="text-sm font-bold text-[#2A2621] uppercase tracking-tight">Edit Profile</p>
                <p className="text-[10px] text-[#5A554E]">Customize details, banner &amp; avatar</p>
              </div>
              <ChevronRight className="h-4 w-4 text-[#5A554E]/40 group-hover:text-[#2A2621]/50 transition-colors shrink-0" />
            </button>
                  ...
                  <div className="p-4 bg-[#f7f7f5] space-y-4">
                    {[
                      ...
                      <div key={field.label} className="space-y-1">
                        <label className="text-[9px] font-bold text-[#5A554E] uppercase tracking-widest">{field.label}</label>
                        <input
                          type={field.type}
                          value={field.value}
                          onChange={e => field.set(e.target.value)}
                          className="w-full bg-white border border-black/[0.08] rounded-xl px-3 py-2 text-xs text-[#2A2621] focus:outline-none focus:border-[#FD5C05] transition-colors"
                        />
                      </div>
                    ))}
                    ...
                    {/* Avatar Customizer */}
                    <div className="space-y-2">
                      <label className="text-[9px] font-bold text-[#5A554E] uppercase tracking-widest block">Choose Avatar Emoji</label>
                        ...
                        {PRESET_AVATARS.map(emoji => (
                          <button
                            ...
                            className={`h-8 w-8 rounded-lg flex items-center justify-center text-sm transition-all border ${
                              editAvatar === emoji
                                ? 'bg-[#FD5C05] border-[#FD5C05] text-[#2A2621] scale-110 shadow-sm'
                                : 'bg-black/[0.02] border-black/[0.05] hover:bg-black/[0.05] text-[#2A2621]'
                            }`}
                          >
                            {emoji}
                          </button>
                      ...
                      <div className="grid grid-cols-2 gap-3 pt-1">
                        <div className="space-y-1">
                          <label className="text-[8px] font-bold text-[#5A554E] uppercase tracking-wider block pl-0.5">Custom Initials/Emoji</label>
                          <input
                            type="text"
                            maxLength={3}
                        ...
                        <div className="space-y-1">
                          <label className="text-[8px] font-bold text-[#5A554E] uppercase tracking-wider block pl-0.5">Or Upload Photo</label>
                          <label className="relative flex items-center justify-center border border-dashed border-black/20 hover:border-black/40 rounded-xl px-3 py-2 text-xs text-[#5A554E] hover:text-[#2A2621] transition-all cursor-pointer bg-white h-9">
                    ...
                    {/* Banner Customizer */}
                    <div className="space-y-2">
                      <label className="text-[9px] font-bold text-[#5A554E] uppercase tracking-widest block">Select Banner Preset</label>
                        ...
                        {PROFILE_BANNERS.map((banner, index) => (
                          <button
                            ...
                            className={`relative aspect-[16/9] rounded-lg overflow-hidden border-2 transition-all ${
                              editBanner === banner
                                ? 'border-[#FD5C05] scale-105 shadow-md'
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pt-1">
                        <div className="space-y-1">
                          <label className="text-[8px] font-bold text-[#5A554E] uppercase tracking-wider block pl-0.5">Paste Custom Image URL</label>
                          <input
                            type="text"
                    ...
                    <div className="flex gap-2 pt-2 border-t border-black/[0.05]">
                      <button
                        onClick={saveEdit}
                        className="flex-1 bg-[#FD5C05] text-[#2A2621] text-xs font-bold uppercase tracking-wider py-2.5 rounded-xl hover:bg-[#FD5C05]/90 transition-colors"
                      >
                        Save changes
                      </button>
                      <button
                        onClick={() => setEditOpen(false)}
                        className="h-10 w-10 rounded-xl bg-black/[0.05] flex items-center justify-center text-[#5A554E] hover:text-[#2A2621] transition-colors"
                      >
                        <X className="h-4 w-4" />
                      </button>
                    </div>
            ...
            >
              <div className="h-9 w-9 rounded-xl bg-black/[0.05] flex items-center justify-center shrink-0">
                <Bell className="h-4 w-4 text-[#2A2621]" />
              </div>
              <div className="flex-1 text-left">
                <p className="text-sm font-bold text-[#2A2621] uppercase tracking-tight">Notifications</p>
                <p className="text-[10px] text-[#5A554E]">Manage your alerts</p>
              </div>
              <ChevronDown className={`h-4 w-4 text-[#5A554E]/40 transition-transform duration-200 shrink-0 ${notifOpen ? 'rotate-180' : ''}`} />
            </button>
                    ...
                    {[
                      ...
                      <div key={item.key} className="flex items-center justify-between gap-3">
                        <div className="flex-1 min-w-0">
                          <p className="text-xs font-bold text-[#2A2621] uppercase tracking-tight">{item.label}</p>
                          <p className="text-[10px] text-[#5A554E] font-normal">{item.desc}</p>
                        </div>
                        <button
                          onClick={() => toggleNotif(item.key)}
                          className={`relative h-6 w-11 rounded-full transition-colors duration-200 shrink-0 ${notifSettings[item.key] ? 'bg-[#FD5C05]' : 'bg-black/[0.12]'}`}
                        >
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              {membershipRequests.filter(r => r.status === 'pending').map(req => (
                <div key={req.id} className="bg-white rounded-2xl p-4 border border-black/[0.04] shadow-sm flex flex-col justify-between gap-4">
                  <div>
                    <span className="text-[9px] font-bold text-[#2A2621] uppercase tracking-widest block">Join Request</span>
                    <h4 className="text-sm font-extrabold text-[#2A2621] uppercase tracking-tight mt-1">{req.studentName}</h4>
                    <p className="text-xs text-[#5A554E] mt-0.5">Wants to join: <strong className="text-[#2A2621]">{req.orgName}</strong></p>
                  </div>
      ...
      <div className="px-5 md:px-10 mt-8 space-y-6 text-left">
        {/* Upcoming events */}
        <div className="space-y-3">
          <div className="text-[11px] font-black text-[#2A2621] uppercase tracking-widest">My Upcoming Experiences</div>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              {upcomingEvents.map(event => (
                <div 
                  ...
                  className="bg-white rounded-2xl p-4 border border-black/[0.04] shadow-sm hover:border-[#FD5C05]/40 transition-all cursor-pointer flex gap-3 items-center"
                >
                  <div className="h-10 w-10 rounded-xl bg-[#FD5C05]/10 border border-[#FD5C05]/20 flex flex-col items-center justify-center shrink-0">
                    <Calendar className="h-4.5 w-4.5 text-[#3B5C00]" />
                  </div>
                  <div className="min-w-0 flex-1">
                    <h4 className="text-xs font-bold text-[#2A2621] uppercase tracking-tight truncate leading-snug">{event.title}</h4>
                    <p className="text-[9px] text-[#5A554E] mt-0.5">{new Date(event.date).toLocaleDateString()} • {event.location}</p>
                  </div>
                </div>
              ))}
            </div>
          ) : (
            <p className="text-xs text-[#5A554E] italic py-4 bg-white rounded-2xl border border-black/[0.04] text-center">No upcoming RSVPs.</p>
          )}
        </div>

        {/* Recently Attended */}
        <div className="space-y-3">
          <div className="text-[11px] font-black text-[#2A2621] uppercase tracking-widest">Recently Attended</div>
          {pastEvents.length > 0 ? (
            <div className="grid gap-4 sm:grid-cols-2">
              {pastEvents.map(event => (
                <div 
                  key={event.id}
                  onClick={() => router.push(`/events/${event.id}`)}
                  className="bg-white rounded-2xl p-4 border border-black/[0.04] shadow-sm hover:border-[#FD5C05]/40 transition-all cursor-pointer flex gap-3 items-center opacity-85 hover:opacity-100"
                >
                  <div className="h-10 w-10 rounded-xl bg-slate-100 border border-[#D8D2BC]/40 flex flex-col items-center justify-center shrink-0">
            ...
            <p className="text-xs text-[#5A554E] italic py-4 bg-white rounded-2xl border border-black/[0.04] text-center">No recently attended events.</p>
          )}
        </div>
      </div>
    </div>
