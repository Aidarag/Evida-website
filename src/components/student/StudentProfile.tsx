  ...
  return (
    <div className="grid gap-6 md:grid-cols-3 max-w-4xl mx-auto">
      ...
      <div className="md:col-span-1 rounded-[28px] border border-white/5 bg-[#121215]/50 p-6 flex flex-col items-center text-center relative overflow-hidden">
        ...
        {/* Avatar */}
        <div className="h-20 w-20 rounded-3xl bg-gradient-to-tr from-[#FF7A1A] to-[#FFD214] text-black font-bold text-3xl flex items-center justify-center shadow-lg shadow-orange-500/10">
          {currentUser.avatar}
        </div>

        <h3 className="text-base font-extrabold text-white mt-4 uppercase">{currentUser.name}</h3>
        <span className="rounded-full bg-[#FF7A1A]/10 border border-[#FF7A1A]/20 px-3 py-0.5 text-[9px] font-bold uppercase tracking-widest text-[#FF7A1A] mt-1.5">
        ...
        <div className="grid grid-cols-3 gap-3 w-full pt-6 mt-6 border-t border-white/5 text-center">
          <div>
            <p className="text-sm font-bold text-white">{totalRsvps}</p>
            <p className="text-[9px] font-bold text-slate-500 uppercase tracking-wide">RSVPs</p>
          </div>
          <div>
            <p className="text-sm font-bold text-white">{totalCreated}</p>
            <p className="text-[9px] font-bold text-slate-500 uppercase tracking-wide">Posts</p>
          </div>
          <div>
            <p className="text-sm font-bold text-white">{totalSaved}</p>
      ...
      <div className="md:col-span-2 space-y-6">
        ...
        <div className="rounded-[28px] border border-white/5 bg-[#121215]/50 p-6 space-y-4">
          <span className="text-[9px] font-bold uppercase text-slate-500">Education Details</span>
        ...
        <div className="rounded-[28px] border border-white/5 bg-[#121215]/50 p-6 space-y-4">
          <span className="text-[9px] font-bold uppercase text-slate-500">Verified Group Memberships</span>
              ...
              userOrgs.map((org) => (
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-bold text-slate-200 uppercase">{org.name}</span>
                  </div>
                  {org.verified && (
