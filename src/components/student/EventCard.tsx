...
export default function EventCard({ event, onClick, onSave, isSaved = false }: EventCardProps) {
  ...
  const bgClass = isGradient ? coverImage : (coverImage ? '' : 'bg-[#FFFDF8]');
  ...
  const getCategoryStyles = (cat?: string) => {
    if (isPromo) return 'bg-[#FF5A1F]/10 text-[#FF5A1F] border-[#FF5A1F]/15';
    ...
    >
      ...
      <div className="p-6 flex flex-col flex-1 justify-between gap-4 text-left">
        <div className="space-y-2 cursor-pointer" onClick={onClick}>
          ...
          <div className="text-[#FF5A1F] text-[10px] font-bold uppercase tracking-widest">
            {formattedDate} • {timeStr}
          </div>

          {/* Event Title */}
          <h3 className="text-[#121212] font-bold text-lg line-clamp-2 leading-tight tracking-tight hover:text-[#FF5A1F] transition-colors" style={{ fontFamily: 'var(--font-display)' }}>
            {event.title}
          </h3>
        ...
        <div className="pt-4 border-t border-black/[0.04] flex items-center justify-between gap-2">
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 bg-white border border-black/10 hover:border-transparent hover:bg-[#FF5A1F] hover:text-white text-[#121212] font-bold text-[10px] uppercase tracking-wider py-1.5 px-3.5 rounded-full transition-all duration-300 shadow-sm cursor-pointer whitespace-nowrap"
